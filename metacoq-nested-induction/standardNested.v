...
(* Inductive is_All (A : Type) (PA: A -> Type) (P : A -> Type) (PP:forall a:A, PA a -> P a -> Type) : forall (xs:list A), is_list _ PA xs -> All_Forall.All P xs -> Type := *)
(*     (1* is_All_nil : is_All A PA P PP (@nil A) (@is_nil A PA) (@All_nil A P). *1) *)
(* | is_All_cons (x:A) (px:PA x) (l:list A) (pl: is_list A PA l) *)
(*     (h: P x) (ph: PP x px h) (h2: All_Forall.All P l) (ph2: is_All A PA P PP l pl h2): *)
(*     is_All A PA P PP (cons x l) (@is_cons A PA x px l pl) (@All_cons A P x l h h2). *)
(*   (1* | is_All_cons : forall (x : A) (l : list A), *1) *)
(*                (1* P x -> All_Forall.All P l -> All_Forall.All P (x :: l) *1) *)
(* Definition allAssumption  := is_All. *)

(* Definition allProof (A:Type) (PA:A -> Type) (HA:forall a, PA a) *)
(*                     (P:A->Type) (PP: forall a:A, PA a -> P a -> Type) (HP: forall (a:A) (h:PA a), HA a -> PP a h ) *)
